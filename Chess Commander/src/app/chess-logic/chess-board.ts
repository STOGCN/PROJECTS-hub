...
export class ChessBoard {
    private chessBoard: (Piece | null)[][]; //กระดานหมากรุก มีตัวอะไรบ้าง
    private readonly chessBoardSize: number = 8; //ขนาดกระดานหมากรุก
    private _playerColor = Color.White; //เริ่มที่สี
    private _safeSquares: SafeSquares; //ตารางที่ปลอดภัย
    private _lastMove: LastMove | undefined; //การเดินล่าสุด
    private _checkState: CheckState = { isInCheck: false }; //สถานะการรุก
    private fiftyMoveRuleCounter: number = 0; //ตัวนับกฎ 50 ตาเดิน

    private _isGameOver: boolean = false; //เกมจบ
    private _gameOverMessage: string | undefined; //ข้อความจบเกม

    private fullNumberOfMoves: number = 1; //จำนวนตาเดินเต็ม
    private threeFoldRepetitionDictionary = new Map<string, number>(); //พจนานุกรมการซ้ำสามครั้ง
    private threeFoldRepetitionFlag: boolean = false; //ธงการซ้ำสามครั้ง

    private _boardAsFEN: string = FENConverter.initalPosition; //กระดานหมากรุกในรูปแบบ FEN
    private FENConverter = new FENConverter(); //ตัวแปลง FEN
    ...
    public get playerColor(): Color { //เรียกผู้เล่นล่าสุด
        return this._playerColor;
    }

    public get chessBoardView(): (FENChar | null)[][] { //บันทึกกระดานหมากรุก
